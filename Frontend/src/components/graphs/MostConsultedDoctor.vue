...
export default {
  ...
  mounted() {
    ...
    for (let patient of patientData) {
      if (labels.has(patient.attending_doctor_name)) {
        labels.set(
          patient.attending_doctor_name,
          labels.get(patient.attending_doctor_name) + 1
        );
      } else {
        labels.set(patient.attending_doctor_name, 1);
      }
    }
    ...
    for (let label of labels.keys()) {
      ...
      if (doc) thresholdData.push(doc.thresholds.attending);
    }
    const colors = [];
